...
export function createLoggingMiddleware(logger: winston.Logger): (rpc: string) => UnaryMiddleware {
  return (rpc) =>
    <Request, Reply>(next: UnaryHandler<Request, Reply>) =>
    async (req: Request, ctx: UnaryContext) => {
      ...
      return next(req, ctx)
        ...
        .finally(() => {
          ...
          const call = ctx.get(callContextKey) as grpc.ServerUnaryCall<Request, Reply>;
